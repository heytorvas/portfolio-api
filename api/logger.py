...
logging_config = {
    ...
    "formatters": {
        "json": {
            ...
            "format":
            "%(asctime)s %(process)s %(levelname)s %(name)s %(module)s %(funcName)s %(lineno)s"
        }
    },
    "handlers": {
        "console": {
...
class RouterLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def _log_response(self, call_next: Callable, request: Request,
    ...
    async def _execute_request(self, call_next: Callable, request: Request,
                               request_id: str) -> Response:
